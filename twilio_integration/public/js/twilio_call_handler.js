...
var onload_script = function(DeviceCtor) {
    ...
    class OutgoingCallPopup extends TwilioCallPopup {
        ...
        async show() {
            if (window.cur_frm && cur_frm.doc && cur_frm.doc.custom_selling_step) {
                const res = await frappe.db.get_value("Selling Step", cur_frm.doc.custom_selling_step, "call_instructions");
                def = (res.message && res.message.call_instructions) || "";
            }
            this.dialog = new frappe.ui.Dialog({
                ...
                fields: [{
                    ...
                    onchange: async() => {
                        ...
                        if (sell_type) {
                            const res = await frappe.db.get_value("Selling Step", sell_type, "call_instructions");
                            this.dialog.set_value("instructions", (res.message && res.message.call_instructions) || "");
                        }
                    }
                }, {
            ...
            $(`<span class="dialpad-icon hide"><a class="btn-open no-decoration" title="${__("Dialpad")}">${frappe.utils.icon("dialpad")}</a></span>`).insertAfter(to_number);
            $(`<div class="dialpad-section hide"></div>`).insertAfter(this.dialog.$wrapper.find(".modal-content"));
    ...
    class IncomingCallPopup extends TwilioCallPopup {
        ...
        get_title(caller_details) {
            ...
            return __("Incoming Call From {0}", [(this.call.parameters && this.call.parameters.From) || "Unknown"]);
        }
        set_dialog_body(caller_details) {
            let caller_details_html = "";
            if (caller_details) {
                for (const [key, value] of Object.entries(caller_details)) {
                    caller_details_html += `<div>${frappe.utils.escape_html(key)}: ${frappe.utils.escape_html(String(value))}</div>`;
                }
            } else {
                caller_details_html += `<div>Phone Number: ${(this.call.parameters && this.call.parameters.From) || ""}</div>`;
            }
    ...
    class DialPad extends OutgoingCallPopup {
        ...
        prepare_dom() {
            ...
            this.wrapper.html(`<i class="dialpad--pointer"></i><div class="dialpad-container"><input class="dialpad-input form-control" readonly="true"><div class="dialpad-keys">${get_keys()}</div></div>`);
        }
        bind_events() {
            const me = this;
